...
mod tests 
{
    ...
    fn test_mesure_div_f64() 
	{
		...
		let V: RMesure = RMesure::scalaire(3.0_f64);

		let resultat_val = 2.0_f64/3.0_f64;
		let resultat_eps = (((U.Val().powf(2.0_f64) * V.Variance()) + (V.Val().powf(2.0_f64) * U.Variance())) / V.Val().powf(4.0_f64)).sqrt();
		let resultat_alf = 95.45_f64;
    ...
    fn test_mesure_div_00() 
	{
		...
		let resultat = division.Val() == (U.Val().signum() * RMESURE_MAX) && division.Eps() == RMESURE_MAX.sqrt() && division.Alpha() == 95.45_f64;
